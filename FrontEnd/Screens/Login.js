...
import { CheckLoginStudentFromServer } from '../Networking/Server'
class Login extends Component {
    ...
    static GetInfoStudent() {
        return global.infoStudent
    }
    static GetInfoClass() {
        return global.infoClass
    }
    ...
    onPress = async () => {
        ...
        {
            this.setState({ isLoading: true });
            let checkValidAccout = await CheckLoginStudentFromServer(this.state.userName, this.state.password)
            this.setState({ isLoading: false });
            if (checkValidAccout.status == 1) {
                global.infoStudent = checkValidAccout.data;
